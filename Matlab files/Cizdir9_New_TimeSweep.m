function [plo]=Cizdir9_New_TimeSweep(Time,lat,dat_i,Nt,lon_plot,per,name,bool_color_num,bool_save,model_name,bool_maps_type)
% plot at a given longitude, time sweep
...
dat_mid = squeeze(dat_i(:,lon_plot,:)); % Size: Nlon, Nlat
Ntime = numel(Time);
time_idx = 1:Ntime;
time_2idx = cat(2,time_idx,time_idx);

NNv=floor((Ntime)/2); % Nt/2
%lm=int8(1:NNv/8:NNv+1); % % NNv/8
lm=(1:floor(NNv/8):NNv+1); % % NNv/8
A1=Cizdir9_Newmod(dat_mid,lm(1),time_2idx,Ntime);
A2=Cizdir9_Newmod(dat_mid,lm(2),time_2idx,Ntime);
A3=Cizdir9_Newmod(dat_mid,lm(3),time_2idx,Ntime);
A4=Cizdir9_Newmod(dat_mid,lm(4),time_2idx,Ntime);
A5=Cizdir9_Newmod(dat_mid,lm(5),time_2idx,Ntime);
A6=Cizdir9_Newmod(dat_mid,lm(6),time_2idx,Ntime);
A7=Cizdir9_Newmod(dat_mid,lm(7),time_2idx,Ntime);
A8=Cizdir9_Newmod(dat_mid,lm(8),time_2idx,Ntime);
A9=Cizdir9_Newmod(dat_mid,lm(9),time_2idx,Ntime);
...
%%%%%%%%%%%%%%%%3D%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Conversion of Time into mean anomaly M
Time = Time*2*pi/Time(end);
lonn=Time*180;%-0.25;
...
if bool_ham
    ...
    subplot(331)
    MAP3D_HAM(Plg,Plt,A1,mi,ma)
    text(0,0,['M=  ',num2str(Time(lm(1))*180/pi),'^o'])
    subplot(332)
    MAP3D_HAM(Plg,Plt,A2,mi,ma)
    text(0,0,['M=  ',num2str(Time(lm(2))*180/pi),'^o'])
    subplot(333)
    MAP3D_HAM(Plg,Plt,A3,mi,ma)
    text(0,0,['M=  ',num2str(Time(lm(3))*180/pi),'^o'])
    subplot(334)
    MAP3D_HAM(Plg,Plt,A4,mi,ma)
    text(0,0,['M=  ',num2str(Time(lm(4))*180/pi),'^o'])
    subplot(335)
    MAP3D_HAM(Plg,Plt,A5,mi,ma)
    text(0,0,['M=  ',num2str(Time(lm(5))*180/pi),'^o'])
    subplot(336)
    MAP3D_HAM(Plg,Plt,A6,mi,ma)
    text(0,0,['M=  ',num2str(Time(lm(6))*180/pi),'^o'])
    subplot(337)
    MAP3D_HAM(Plg,Plt,A7,mi,ma)
    text(0,0,['M=  ',num2str(Time(lm(7))*180/pi),'^o'])
    subplot(338)
    MAP3D_HAM(Plg,Plt,A8,mi,ma)
    text(0,0,['M=  ',num2str(Time(lm(8))*180/pi),'^o'])
    subplot(339)
    MAP3D_HAM(Plg,Plt,A9,mi,ma)
    text(0,0,['M=  ',num2str(Time(lm(9))*180/pi),'^o'])
    c = colorbar;
...
% subplot(331)
%  MAP3D_MILLER(Plg,Plt,A1,mi,ma)
%  text(0,0,['M=  ',num2str(lon(lm(1))*180/pi),'^o'])
% subplot(332)
%  MAP3D_MILLER(Plg,Plt,A2,mi,ma)
%  text(0,0,['M=  ',num2str(lon(lm(2))*180/pi),'^o'])
% subplot(333)
%  MAP3D_MILLER(Plg,Plt,A3,mi,ma)
%  text(0,0,['M=  ',num2str(lon(lm(3))*180/pi),'^o'])
% subplot(334)
%  MAP3D_MILLER(Plg,Plt,A4,mi,ma)
%  text(0,0,['M=  ',num2str(lon(lm(4))*180/pi),'^o'])
% subplot(335)
%  MAP3D_MILLER(Plg,Plt,A5,mi,ma)
%  text(0,0,['M=  ',num2str(lon(lm(5))*180/pi),'^o'])
% subplot(336)
%  MAP3D_ORTHO(Plg,Plt,A6,mi,ma)
%  text(0,0,['M=  ',num2str(lon(lm(6))*180/pi),'^o'])
% subplot(337)
%  MAP3D_ORTHO(Plg,Plt,A7,mi,ma)
%  text(0,0,['M=  ',num2str(lon(lm(7))*180/pi),'^o'])
%  subplot(338)
%  MAP3D_ORTHO(Plg,Plt,A8,mi,ma)
%  text(0,0,['M=  ',num2str(lon(lm(8))*180/pi),'^o'])
%  subplot(339)
%  MAP3D_STEREOGRAPHIC(Plg,Plt,A9,mi,ma)
%  text(0,0,['M=  ',num2str(lon(lm(9))*180/pi),'^o'])
%  colorbar('h')
%
...
if bool_miller
    ...
    subplot(333)
    MAP3D_MILLER(Plg,Plt,A3,mi,ma)
    text(0,0,['M=  ',num2str(Time(lm(3))*180/pi),'^0'])
    subplot(334)
    MAP3D_MILLER(Plg,Plt,A4,mi,ma)
    text(0,0,['M=  ',num2str(Time(lm(4))*180/pi),'^0'])
    subplot(335)
    MAP3D_MILLER(Plg,Plt,A5,mi,ma)
    text(0,0,['M=  ',num2str(Time(lm(5))*180/pi),'^o'])
    subplot(336)
...
if bool_ortho
    ...
    text(0,0,['M=  ',num2str(Time(lm(9))*180/pi),'^0'])
    c = colorbar;
    if bool_criterion_find && ~bool_color_num
        c = colorbar('Ticks',array_bar,'TickLabels',label_bar);
    end
...
if bool_ortho_3D
    ...
    MAP3D_ORTHO(Plg,Plt,A1,mi,ma)
    text(-0.1,0,'\lambda= ')
    text(0.0,0,num2str(Time(lm(1))*180/pi))
    text(0.0,0,'  ^o')
    c = colorbar;
    if bool_criterion_find && ~bool_color_num
        c = colorbar('Ticks',array_bar,'TickLabels',label_bar);
...
if bool_stereo
    ...
    text(0,0,['M=  ',num2str(Time(lm(6))*180/pi),'^0']);
    subplot(3,3,7);
